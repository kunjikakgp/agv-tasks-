...
I1 = cv.cvtColor(cv.imread(r"c:\Users\USER\Downloads\im1.png"), cv.COLOR_BGR2GRAY).astype(np.float32)
I2 = cv.cvtColor(cv.imread(r"c:\Users\USER\Downloads\im2.png"), cv.COLOR_BGR2GRAY).astype(np.float32)

intrinsics = np.load(r'c:\Users\USER\Downloads\intrinsics.npz')
K1, K2 = intrinsics['K1'], intrinsics['K2']

extrinsics = np.load('dataextrinsics.npz')
R1, R2 = extrinsics['R1'], extrinsics['R2']
t1, t2 = extrinsics['t1'], extrinsics['t2']

# 2. Rectify the images and save the paramters

M1, M2 = sub.rectify_pair(K1, K2, R1, R2, t1, t2)
...
corresp = np.load(r'c:\Users\USER\Downloads\some_corresp.npz')
